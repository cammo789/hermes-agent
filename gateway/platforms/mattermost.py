...
class MattermostAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Command prefix for this adapter. Defaults to "\" to avoid Mattermost's
        # built-in slash-command interceptor (/ commands get eaten by the client).
        # Can be overridden per-channel via environment variable.
        self._cmd_prefix: str = os.getenv("MATTERMOST_COMMAND_PREFIX", "\\")
    ...
    async def _handle_ws_event(self, event: Dict[str, Any]) -> None:
        ...
        if message_text.startswith(self._cmd_prefix):
            msg_type = MessageType.COMMAND
            # Strip the prefix so the command handler gets clean text (\new → new)
            message_text = message_text[len(self._cmd_prefix):]
